'''
IMPORTANT NOTES:
In the .m2fs file, the first and last RA/DEC represent a reference slit at the bottom of the mask and the center of the mask respectively.
...
#############################################################
instrument = 'M2FS'
path_to_data = 'C:\\Users\\kremin\\OneDrive\\Research\\M2FSReductions'
mask_name = 'A02'
shoe_dirnames = ['blue','red']
...
data_dir = os.path.join(path_to_data,mask_name,shoe_dirnames[1])
...
for lamp in cal_lamp:
    if lamp in ['Xenon','Neon','Argon','HgNe']:
        wm_itter_air, fm_itter = np.loadtxt(os.path.join(code_dir,'osmos_{}.dat'.format(lamp)), usecols=(0, 2), unpack=True)
        wm__itter_vac = air_to_vacuum(wm_itter_air)
        wm.extend(wm__itter_vac)
        fm.extend(fm_itter)
...
if mask_type == 'slit':
    ...
    Gal_dat = pd.DataFrame({
                               'RA': RA[1:SLIT_WIDTH.size], 'DEC': DEC[1:SLIT_WIDTH.size], 'SLIT_WIDTH': SLIT_WIDTH[1:],
                               'SLIT_LENGTH': SLIT_LENGTH[1:], 'SLIT_X': SLIT_X[1:], 'SLIT_Y': SLIT_Y[1:]
                           })
    ...
    for fil in os.listdir(cluster_dir):
        ...
        for fil in os.listdir(cluster_dir + os_slash + 'science' + os_slash):
            filerootlist = re.findall('[br]([0-9][0-9][0-9][0-9])c[1-4]_b.fits',
                                      fil)  # re.findall('([\w]*c)[1-9]_b.fits',fil)
    ...
    # remove throw away rows and dump into Gal_dat dataframe
    Gal_dat = pd.DataFrame({
                               'RA': target_table['ra'], 'DEC': target_table['dec'], 'SLIT_WIDTH': SLIT_WIDTH,
                               'SLIT_LENGTH': SLIT_LENGTH, 'SLIT_X': SLIT_X, 'SLIT_Y': SLIT_Y
                           })
...
####################
p = subprocess.Popen(
    'ds9 ' + cluster_dir + image_file + ' -geometry 1200x900 -scale sqrt -scale mode zscale -fits ' + cluster_dir + 'arcs' + os_slash +
    arcfiles[0], shell=True)
...
d = pyds9.DS9(target='Spectral_Reduction',start=True,wait=10) # start pyds9 and set parameters
d.set("file {}".format(os.path.join(cluster_dir,image_file)))
d.set('frame 1')
d.set('single')
d.set('geometry 1200x900')
d.set('scale sqrt')
...
if os.path.isfile(cluster_dir + clus_id + '_slittypes.pkl'):
    reassign = input(
        'Detected slit types file in path. Do you wish to use this (y) or remove and re-assign slit types (n)? ')
if reassign == 'n':
    slit_type = {}
...
d.set('frame 2')
d.set('fits {}'.format(os.path.join(cluster_dir, 'arcs',arcfiles[0])))
...
if os.path.isfile(cluster_dir + 'science' + os_slash + clus_id + '_science.cr.fits'):
    redo = input(
        'Detected cosmic ray filtered file exists. Do you wish to use this (y) or remove and re-calculate (n)? ')
if redo == 'n':
    try:
...
if redo == 'n':
    ...
    flatfits_c = pyfits.open(cluster_dir + 'flats' + os_slash + clus_id + '_flat.cr.fits')[0]
...
if redo == 'n':
    ...
    arcfits_c = pyfits.open(cluster_dir + 'arcs' + os_slash + clus_id + '_arc.cr.fits')[0]
...
if os.path.isfile(cluster_dir + clus_id + '_slit_pos_qual.tab'):
    reassign = input(
        'Detected slit position and quality file in path. Do you wish to use this (y) or remove and re-adjust (n)? ')
if reassign == 'n':
                ...
                while not good and loops <= 3:
                    ...
                    for n_string in newpos_str:
                        if n_string[:3] == 'box':
                            ...
                            ##
                            try:
                                science_spec, arc_spec, gal_spec, gal_cuts, lower_lim, upper_lim = slit_find(
                                    flatfits_c.data[
                                    FINAL_SLIT_Y[i] - SLIT_WIDTH[i] / 2.0:FINAL_SLIT_Y[i] + SLIT_WIDTH[i] / 2.0, :],
                                    scifits_c.data[
                                    FINAL_SLIT_Y[i] - SLIT_WIDTH[i] / 2.0:FINAL_SLIT_Y[i] + SLIT_WIDTH[i] / 2.0, :],
                                    arcfits_c.data[
                                    FINAL_SLIT_Y[i] - SLIT_WIDTH[i] / 2.0:FINAL_SLIT_Y[i] + SLIT_WIDTH[i] / 2.0, :],
                                    lower_lim, upper_lim)
                                spectra[keys[i]] = {
                                    'science_spec': science_spec, 'arc_spec': arc_spec, 'gal_spec': gal_spec,
                                    'gal_cuts': gal_cuts
                                }
    ...
    print(FINAL_SLIT_X)
    np.savetxt(clus_id + os_slash + clus_id + '_slit_pos_qual.tab',
               np.array(list(zip(FINAL_SLIT_X, FINAL_SLIT_Y, SLIT_WIDTH, good_spectra)),
                        dtype=[('float', float), ('float2', float), ('int', int), ('str', '|S1')]), delimiter='\t',
               fmt='%10.2f %10.2f %3d %s')
    pkl.dump(spectra, open(cluster_dir + clus_id + '_reduced_spectra.pkl', 'wb'))
else:
    FINAL_SLIT_X, FINAL_SLIT_Y, SLIT_WIDTH = np.loadtxt(cluster_dir + clus_id + '_slit_pos_qual.tab', dtype='float',
...
Gal_dat['FINAL_SLIT_X'], Gal_dat['FINAL_SLIT_Y'], Gal_dat['SLIT_WIDTH'], Gal_dat[
    'good_spectra'] = FINAL_SLIT_X, FINAL_SLIT_Y, SLIT_WIDTH, good_spectra
...
if os.path.isfile(cluster_dir + clus_id + '_stretchshift.tab'):
    reassign = input(
        'Detected file with stretch and shift parameters for each spectra. Do you wish to use this (y) or remove and re-adjust (n)? ')
if reassign == 'n':
    # create write file
    ...
    shift_est = 4.71e-6 * (Gal_dat['FINAL_SLIT_X'] - 2500.0) ** 2 + 4.30e-6 * (
                Gal_dat['FINAL_SLIT_Y'] - 2000) ** 2 + 4469.72
    stretch_est = -9.75e-9 * (Gal_dat['FINAL_SLIT_X'] - 1800.0) ** 2 - 2.84e-9 * (
                Gal_dat['FINAL_SLIT_Y'] - 2000) ** 2 + 0.7139
    ...
    while ii <= stretch.size:
        if good_spectra[ii] == 'y':
            ...
            d.set('regions command {box(2000 ' + str(Gal_dat.FINAL_SLIT_Y[ii]) + ' 4500 ' + str(
                Gal_dat.SLIT_WIDTH[ii]) + ') #color=green highlite=1}')

            # initial stretch and shift
            stretch_est[ii], shift_est[ii], quad_est[ii] = interactive_plot(p_x, f_x, stretch_est[ii], shift_est[ii],
                                                                            quad_est[ii], cube_est[ii], fourth_est[ii],
                                                                            fifth_est[ii],
                                                                            Gal_dat.FINAL_SLIT_X_FLIP[ii], wm, fm)
            ...
            xspectra = fifth_est[ii] * (p_x - Gal_dat.FINAL_SLIT_X_FLIP[ii]) ** 5 + fourth_est[ii] * (
                        p_x - Gal_dat.FINAL_SLIT_X_FLIP[ii]) ** 4 + cube_est[ii] * (
                                   p_x - Gal_dat.FINAL_SLIT_X_FLIP[ii]) ** 3 + quad_est[ii] * (
                                   p_x - Gal_dat.FINAL_SLIT_X_FLIP[ii]) ** 2 + stretch_est[ii] * (
            ...
            browser = LineBrowser(fig, ax, est_features, wm, fm, p_x - Gal_dat.FINAL_SLIT_X_FLIP[ii],
                                  Gal_dat.FINAL_SLIT_X_FLIP[ii], vlines, fline, xspectra, yspectra, peaks, fxpeak,
            ...
            params, pcov = curve_fit(polyfour,
                                     (np.sort(browser.line_matches['peaks_p']) - Gal_dat.FINAL_SLIT_X_FLIP[ii]),
                                     np.sort(browser.line_matches['lines']),
                                     p0=[shift_est[ii], stretch_est[ii], quad_est[ii], cube_est[ii], 1e-12, 1e-12])
            ...
            wave_model = params[0] + params[1] * (p_x - Gal_dat.FINAL_SLIT_X_FLIP[ii]) + params[2] * (
                        p_x - Gal_dat.FINAL_SLIT_X_FLIP[ii]) ** 2 + params[3] * (
                                     p_x - Gal_dat.FINAL_SLIT_X_FLIP[ii]) ** 3.0 + params[4] * (
                                     p_x - Gal_dat.FINAL_SLIT_X_FLIP[ii]) ** 4.0 + params[5] * (
                                     p_x - Gal_dat.FINAL_SLIT_X_FLIP[ii]) ** 5.0
            spectra[keys[ii]]['wave'] = wave_model
            spectra[keys[ii]]['wave2'] = wave_model[p_x >= np.sort(browser.line_matches['peaks_p'])[0]]
            spectra[keys[ii]]['gal_spec2'] = (
            (np.array(spectra[keys[ii]]['gal_spec']).T[::-1])[p_x >= np.sort(browser.line_matches['peaks_p'])[0]]).T

            flu = f_x - np.min(f_x)
            flu = flu[::-1][p_x >= np.sort(browser.line_matches['peaks_p'])[0]]
            Flux = flu / signal.medfilt(flu, 201)
            fifth[ii], fourth[ii], cube[ii], quad[ii], stretch[ii], shift[ii] = params[5], params[4], params[3], params[
                2], params[1], params[0]
    ...
    for i in range(ii, len(Gal_dat)):
        ...
        if Gal_dat.good_spectra[i] == 'y':
            ...
            if not skipgal:
                ...
                d.set('regions command {box(2000 ' + str(Gal_dat.FINAL_SLIT_Y[i]) + ' 4500 ' + str(
                    Gal_dat.SLIT_WIDTH[i]) + ') #color=green highlite=1}')
                ...
                stretch_est[i], shift_est[i], quad_est[i] = interactive_plot(p_x, f_x, stretch_est[i], shift_est[i],
                                                                             ...
                                                                             fifth_est[i], Gal_dat.FINAL_SLIT_X_FLIP[i],
                                                                             wm, fm)
                est_features = [fifth_est[i], fourth_est[i], cube_est[i], quad_est[i], stretch_est[i], shift_est[i]]

                # run peak identifier and match lines to peaks
                line_matches = {'lines': [], 'peaks_p': [], 'peaks_w': [], 'peaks_h': []}
                xspectra = fifth_est[i] * (p_x - Gal_dat.FINAL_SLIT_X_FLIP[i]) ** 5 + fourth_est[i] * (
                            p_x - Gal_dat.FINAL_SLIT_X_FLIP[i]) ** 4 + cube_est[i] * (
                                       p_x - Gal_dat.FINAL_SLIT_X_FLIP[i]) ** 3 + quad_est[i] * (
                                       p_x - Gal_dat.FINAL_SLIT_X_FLIP[i]) ** 2 + stretch_est[i] * (
                                       p_x - Gal_dat.FINAL_SLIT_X_FLIP[i]) + shift_est[i]
                fydat = f_x[::-1] - signal.medfilt(f_x[::-1], 171)  # used to find noise
                fyreal = (f_x[::-1] - f_x.min()) / 10.0
                peaks = argrelextrema(fydat, np.greater)  # find peaks
                fxpeak = xspectra[peaks]  # peaks in wavelength
                ...
                yspectra = (f_x[::-1] - f_x.min()) / 10.0
                fline, = plt.plot(xspectra, yspectra, 'b', lw=1.5, picker=5)
                estx = quad_est[i] * (line_matches['peaks_p'] - Gal_dat.FINAL_SLIT_X_FLIP[i]) ** 2 + stretch_est[i] * (
                            line_matches['peaks_p'] - Gal_dat.FINAL_SLIT_X_FLIP[i]) + shift_est[i]

                browser = LineBrowser(fig, ax, est_features, wm, fm, p_x - Gal_dat.FINAL_SLIT_X_FLIP[i],
                                      Gal_dat.FINAL_SLIT_X_FLIP[i], vlines, fline, xspectra, yspectra, peaks, fxpeak,
                                      fxrpeak, fypeak, line_matches, cal_states)
                fig.canvas.mpl_connect('button_press_event', browser.onclick)
                fig.canvas.mpl_connect('key_press_event', browser.onpress)
                finishax = plt.axes([0.83, 0.85, 0.15, 0.1])
                ...
                try:
                    params, pcov = curve_fit(polyfour,
                                             (np.sort(browser.line_matches['peaks_p']) - Gal_dat.FINAL_SLIT_X_FLIP[i]),
                                             np.sort(browser.line_matches['lines']),
                ...
                # make calibration and clip on lower anchor point. Apply to Flux as well
                wave_model = params[0] + params[1] * (p_x - Gal_dat.FINAL_SLIT_X_FLIP[i]) + params[2] * (
                            p_x - Gal_dat.FINAL_SLIT_X_FLIP[i]) ** 2 + params[3] * (
                                         p_x - Gal_dat.FINAL_SLIT_X_FLIP[i]) ** 3.0 + params[4] * (
                                         p_x - Gal_dat.FINAL_SLIT_X_FLIP[i]) ** 4.0 + params[5] * (
                                         p_x - Gal_dat.FINAL_SLIT_X_FLIP[i]) ** 5.0
                spectra[keys[i]]['wave'] = wave_model
                spectra[keys[i]]['wave2'] = wave_model[p_x >= np.sort(browser.line_matches['peaks_p'])[0]]
                spectra[keys[i]]['gal_spec2'] = (
                (np.array(spectra[keys[i]]['gal_spec']).T[::-1])[p_x >= np.sort(browser.line_matches['peaks_p'])[0]]).T

                flu = f_x[p_x >= np.sort(browser.line_matches['peaks_p'])[0]] - np.min(
                    f_x[p_x >= np.sort(browser.line_matches['peaks_p'])[0]])
                flu = flu[::-1]
                Flux = flu / signal.medfilt(flu, 201)
                fifth[i], fourth[i], cube[i], quad[i], stretch[i], shift[i] = params[5], params[4], params[3], params[
                    2], params[1], params[0]
    ...
    xslit, yslit, shift, stretch, quad, cube, fourth, fifth, wd = np.loadtxt(
        cluster_dir + clus_id + '_stretchshift.tab', dtype='float', usecols=(0, 1, 2, 3, 4, 5, 6, 7, 8), unpack=True)
...
Gal_dat['shift'], Gal_dat['stretch'], Gal_dat['quad'], Gal_dat['cube'], Gal_dat['fourth'], Gal_dat[
    'fifth'] = shift, stretch, quad, cube, fourth, fifth
...
for k in range(len(Gal_dat)):
    if Gal_dat.slit_type[k] == 'g' and Gal_dat.good_spectra[k] == 'y':
        ...
        if not skipgal:
            ...
            cut = F1.copy()
            W = fftpack.rfftfreq(spectra[keys[k]]['wave2'].size,
                                 d=spectra[keys[k]]['wave2'][1001] - spectra[keys[k]]['wave2'][1000])
            ...
            d.set('regions command {box(2000 ' + str(Gal_dat.FINAL_SLIT_Y[k]) + ' 4500 ' + str(
                Gal_dat.SLIT_WIDTH[k]) + ') #color=green highlite=1}')
            redshift_est[k], cor[k], ztest, corr_val, qualityval['Clear'][k] = R.redshift_estimate(early_type_wave,
                                                                                                   early_type_flux,
                                                                                                   spectra[keys[k]][
                                                                                                       'wave2'],
                                                                                                   Flux_science2,
...
Gal_dat['est_z'], Gal_dat['cor'], Gal_dat['HSN'], Gal_dat['KSN'], Gal_dat['GSN'], Gal_dat[
    'quality_flag'] = redshift_est, cor, HSN, KSN, GSN, qualityval['Clear']
