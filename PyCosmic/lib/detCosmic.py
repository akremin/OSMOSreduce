...
def detCos(image,  out_mask, out_clean,  rdnoise, sigma_det=5, rlim=1.2, iter=5, fwhm_gauss=2.0, replace_box=[5,5],  replace_error=1e10, increase_radius=0, gain=1.0, verbose=False, parallel=True,return_data=False):
	"""
	...
	if (gain!=1.0) and verbose==True:
	  print('Convert image from ADUs to electrons using a gain factor of {}'.format(gain))
	...
	if verbose==True:
		print('A value of {} is used for the electron read-out noise.'.format(rdnoise))
	...
	for i in range(iterations):
		...
		if verbose:
			...
			print('Total number of detected cosmics: {} out of {} pixels'.format(numpy.sum(select), dim[0]*dim[1]))
	...
	if verbose==True:
		print('Cleaned image is stored in file: {}'.format(out_clean))
		print('Cosmics mask is stored in file: {}'.format(out_mask))
