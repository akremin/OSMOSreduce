...
def find_apperatures(fibermap, camera='r', deadfibers=None, resol_factor=100., nvertslices=2 ** 6,
    ...
    for itti, start, end in zip(np.arange(len(starts)), starts, ends):
        ...
        apperatures['tetri'][tetnum] = tetris

    return apperatures


def cutout_2d_apperatures(image, apperatures):
    camera = apperatures['camera']
    resol_factor = apperatures['resolution_factor']

    all_cutouts = {}

    for i, tetris in apperatures['tetri'].items():
